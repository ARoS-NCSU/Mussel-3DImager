...
#define SERVO_1           13
#define TURN_TIME         82    // TURN_TIME is time in ms servo will rotate for
#define SERVO_STOP        95    // Servo Stops when 95 is written to it
#define SERVO_MOVE        60    // Value between 0 and 95. Closer to 0 rotates faster 
...
void SendCapturedPhotos() {
  ...
  if (client.connect(host, 443)) {
    ...
    if (LED_Flash_ON == true) digitalWrite(FLASH_LED, LOW);
...
void setup() {
  ...
  int connecting_process_timed_out = 40; //--> 20 = 20 seconds.
...
void loop() {
  ...
  if (currentMillis - previousMillis >= Interval) {
    ...
    servo1.write(SERVO_MOVE);   // Servo rotates at specified speed
    delay(TURN_TIME);           // Servo continues rotating for specified time
    servo1.write(SERVO_STOP);   // Servo Stops 
    Serial.println();
